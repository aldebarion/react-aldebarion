  ...
  render() {
    ...
    return (
      <Page title="Input" description="Inputs are the main way to enter information. So they need to be very clean, efficient and scalable.">
        ...
        <Section index={7} title="Read only">
          <Example>
            <AdInput defaultValue="Read only input" readOnly />
          </Example>
        </Section>
        <Section index={9} title="With an icon" description="We sometimes say a picture is worth a thousand words. When the context and the placeholder are not enough, use a icon to describe what the application needs from the user.">
          <Example>
            <AdInput placeholder="Input with icon" icon="email" />
          </Example>
        </Section>
        <Section index={12} title="With an prefix" description="It is also possible to replace the icon for a prefix when it helps the user to start writing.">
          <Example>
            <AdInput placeholder="Input with prefix" prefix="http://" />
          </Example>
        </Section>
        <Section index={15} title="Integrated button" description="When the data needs to be submitted to be taken into account, a button can be directly linked to the input.">
          <Example>
        ...
        <Section index={24} title="Mixed input" description="Of course, it is also possible to mix all previous feature together.">
          <Example>
            <AdInput placeholder="Only email accepted" showSubmit type="email" icon="email" />
          </Example>
        </Section>
        <Section index={27} title="Animated input" description="In order to avoid to overload user attention, it is suggested to make your components appearing progressively.">
          <Example>
